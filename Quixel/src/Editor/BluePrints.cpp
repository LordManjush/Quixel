...
namespace Quixel
{
    namespace BluePrints
    {
        ...
        ImU32 evaluate(const Quixel::BluePrints::Graph<Node>& graph, const int root_node)
        {
            ...
            void show()
            {
                imnodes::BeginNodeEditor();
                {
                    ...
                    if (ImGui::BeginPopup("New Node"))
                    {
                        ...
                        if (ImGui::MenuItem("substrace"))
                        {
                            const Node value(NodeType::value, 0.f);
                            const Node op(NodeType::substract);

                            UiNode ui_node;
                            ui_node.type = UiNodeType::substract;
                            ui_node.ui.add.lhs = graph_.insert_node(value);
                            ui_node.ui.add.rhs = graph_.insert_node(value);
                            ui_node.id = graph_.insert_node(op);
                        ...
                        {
                            const Node value(NodeType::value, 0.f);
                            const Node out(NodeType::setPosition);

                            UiNode ui_node;
                            ui_node.type = UiNodeType::setPosition;
                            ui_node.ui.setPosition.x = graph_.insert_node(value);
                            ui_node.ui.setPosition.y = graph_.insert_node(value);
                            ui_node.id = graph_.insert_node(out);

                            graph_.insert_edge(ui_node.id, ui_node.ui.setPosition.x);
                            graph_.insert_edge(ui_node.id, ui_node.ui.setPosition.y);
                ...
                for (const UiNode& node : nodes_)
                {
                    ...
                    break;
                    case UiNodeType::substract:
                    {
                        const float node_width = 100.f;
                        imnodes::BeginNode(node.id);

                        imnodes::BeginNodeTitleBar();
                        ImGui::TextUnformatted("substract");
                        imnodes::EndNodeTitleBar();
                        {
                            imnodes::BeginInputAttribute(node.ui.add.lhs);
                            const float label_width = ImGui::CalcTextSize("left").x;
                    ...
                    break;
                    case UiNodeType::setPosition:
                    {
                        const float node_width = 100.0f;
                        imnodes::BeginNode(node.id);

                        imnodes::BeginNodeTitleBar();
                        ImGui::TextUnformatted("Set Position");
                        imnodes::EndNodeTitleBar();

                        ImGui::Dummy(ImVec2(node_width, 0.f));
                        {
                            imnodes::BeginInputAttribute(node.ui.setPosition.x);
                            const float label_width = ImGui::CalcTextSize("x").x;
                            ImGui::TextUnformatted("X");
                            if (graph_.num_edges_from_node(node.ui.setPosition.x) == 0ull)
                            {
                                ImGui::SameLine();
                                ImGui::PushItemWidth(node_width - label_width);
                                ImGui::DragFloat(
                                    "##hidelabel", &graph_.node(node.ui.setPosition.x).value, 0.01f, 0.f, 1.0f);
                                ImGui::PopItemWidth();
                            }
                            imnodes::EndInputAttribute();
                        }
                        ...
                        {
                            imnodes::BeginInputAttribute(node.ui.setPosition.y);
                            const float label_width = ImGui::CalcTextSize("y").x;
                            ImGui::TextUnformatted("Y");
                            if (graph_.num_edges_from_node(node.ui.setPosition.y) == 0ull)
                            {
                                ImGui::SameLine();
                                ImGui::PushItemWidth(node_width - label_width);
                                ImGui::DragFloat(
                                    "##hidelabel", &graph_.node(node.ui.setPosition.y).value, 0.01f, 0.f, 1.f);
                                ImGui::PopItemWidth();
                            }
                            imnodes::EndInputAttribute();
                        }
                    ...
                    if (num_selected > 0 && ImGui::IsKeyReleased(ImGuiKey_X))
                    {
                        ...
                        for (const int node_id : selected_nodes)
                        {
                            ...
                            case UiNodeType::multiply:
                                ...
                                break;
                            case UiNodeType::setPosition:
                                graph_.erase_node(iter->ui.setPosition.x);
                                graph_.erase_node(iter->ui.setPosition.y);
                                root_node_id_ = -2;
                                break;
                            case UiNodeType::sine:
        ...
        private:
            ...
            Quixel::BluePrints::Graph<Node>            graph_;
            std::vector<UiNode>    nodes_;
            int                    root_node_id_;
        };

        static Quixel::BluePrints::BluePrintEditor color_editor;
